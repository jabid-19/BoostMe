import React from 'react'

const Traffic = ({ trafficHeading }) => {
  return (
    <div className="flex items-center flex-col justify-center w-auto mx-6 mt-12 mb-20 lg:mt-36 lg:mb-0 2xl:w-[1400px] 2xl:mx-auto">
      <div data-aos="zoom-in-down" className="w-[90%] lg:w-2/3">
        <h1 className="text-4xl text-center font-semibold break-words text-neutral lg:tracking-wide xl:text-5xl">
          {trafficHeading.titleStart}{' '}
          <span className="text-primary">{trafficHeading.titlePrimary}</span>{' '}
          {trafficHeading.titleMiddle}{' '}
          <span className="text-secondary">{trafficHeading.titleSecondary}</span>{' '}
          {trafficHeading.titleLast}
          {/* Build a loyal and <span className="text-primary">engaged</span> audience on
          <br />
          <span className="text-secondary">social media</span> */}
        </h1>
      </div>
      <div data-aos="zoom-in-up" className="w-[90%] flex flex-col items-center lg:w-2/4">
        <p className="text-neutral text-center text-base mt-10">
          {trafficHeading.description}
          {/* Start Page is a simple, beautiful, flexible link in bio page that brings together all of
          your best content in one place. Share it in your bios on Instagram, YouTube, TikTok, or
          any other social profiles. */}
        </p>
        {/* <button className="btn btn-wide mt-10 btn-primary mx-auto rounded-full text-white normal-case md:mx-0">
          Start 14 days Free Trial
        </button> */}
      </div>
    </div>
  )
}

export default Traffic
